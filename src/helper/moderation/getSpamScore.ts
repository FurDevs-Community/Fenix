...
export const getSpamScore = async (client: HozolClient, message: Message) => {
    ...
    const messageTime =
        message.cleanContent.length / antiSpamSettings.charactersPerSecond;
    ...
    messagesFromAuthor.each((msg) => {
        const timediff = moment(message.createdAt).diff(
        ...
        const similarity = compareTwoStrings(
            `${message.cleanContent || ''}${JSON.stringify(
                message.embeds
            )}${JSON.stringify(message.attachments.array())}`,
            `${msg.cleanContent || ''}${JSON.stringify(
                msg.embeds
            )}${JSON.stringify(msg.attachments.array())}`
        );
                    ...
                    (1 +
                        0.1 *
                            (message.cleanContent
                                ? message.cleanContent.length / 100
                                : 0))
            );
            score += tempScore;
    ...
    const patternScore =
        message.cleanContent.length > 0
            ? newstring.length / message.cleanContent.length
            : 1;
    ...
    const afterFunction = () => {
        ...
        const isMuted =
            message.member &&
            message.guild &&
            message.member.roles.cache.get(settings.muteRole);

        // If this is not a less strict channel, add 0.5 to the multiplier.
        if (
            antiSpamSettings.antispamLessStrictChannels.includes(
                message.channel.id
            )
        )
        ...
        if (typeof message.member !== 'undefined') {
            ...
            message.member?.roles.cache
                .filter((role) => {
                    return (
                        antiSpamSettings.antispamLessStrictRoles.indexOf(
                            role.id
                        ) !== -1
                    );
                })
            ...
            if (!lessStrict)
                multiplier += antiSpamSettings.lessStrictRoleMultiplier;
        }
        ...
        const flagChannel = <TextChannel>(
            message.client.channels.cache.get(settings.flagChannel)
        );
        ...
        if (score > antiSpamSettings.decayFast) {
            if (flagChannel) {
                ...
                flagChannel.send(
                    '⚠️ A message with **__high amounts__** of Spam Score was sent!, Please Investigate',
                    {
                        embed,
                    }
                );
