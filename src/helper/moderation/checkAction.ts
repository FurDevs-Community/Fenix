...
export async function checkActions(moderation: IModeration[], action: string): Promise<Boolean> {
    ...
    if (moderation) {
        ...
        if (records && records.length > 0) {
            records.map((record) => {
                const disciplines = <IDiscipline[]>(<unknown>record.populate('discipline'));
                if (disciplines && disciplines.length > 0) {
                    const _record = disciplines.find(
                        (discipline: any) => discipline.action === action && discipline.status === 'active'
                    );
