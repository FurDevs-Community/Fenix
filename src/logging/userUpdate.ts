...
module.exports = class extends Event {
    ...
    async run(oldUser: User, newUser: User) {
        ...
        client.guilds.cache
            .filter((guild) => guild.members.resolve(newUser.id) as unknown as boolean)
            .each(async (guild) => {
                ...
                if (!oldUser.partial && oldUser.avatar !== newUser.avatar) {
                    const avatarTag = new MessageEmbed()
                        ...
                        .setDescription('Old avatar is at the bottom. New avatar is in the thumbnail.')
                        .setColor('#6610f2')
                        .setTimestamp()
                        .setFooter(`User ID: ${newUser.id}`, `${oldUser.displayAvatarURL({ dynamic: true })}`)
