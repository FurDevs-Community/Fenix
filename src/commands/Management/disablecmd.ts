...
module.exports = class extends Command {
    ...
    async run(message: Message, args: string[], client: HozolClient) {
        ...
        if (cmd.name === 'enablecmd' || cmd.name === 'disableCMD') throw new Error('You cannot disable these commands');
        const newDisabledCommands = [...settings.disabledCommands, cmd.name];
        try {
            await Guild.findOneAndUpdate({ guildID: message.guild.id }, { disabledCommands: newDisabledCommands });
        } catch (e) {
