...
module.exports = class extends Command {
    ...
    constructor(file: any) {
        super(file, {
            ...
            cooldown: 0,
            description: "View the Bot's current prefix or change the bot's behaviors by changing it's prefix",
            enabled: true,
            extendedHelp:
    ...
    async run(message: Message, args: string[], client: HozolClient) {
        ...
        if (!prefix) {
            ...
            if (prefix.length > 5) {
                throw new Error('The prefix is too long please keep it below 5 character.');
            } else if (prefix === settings.prefix) {
                throw new Error("You cannot set a prefix that's already your current prefix");
            } else {
