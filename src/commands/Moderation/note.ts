...
module.exports = class extends Command {
    ...
    async run(message: Message, args: string[], client: Client) {
        if (!args[0]) throw new Error('You must provide a user you wanna note about');
        const user = await usernameResolver(message, args[0]);
        if (!args[1]) throw new Error('You must provide something to note about on that user');
        const note = args.slice(1).join(' ');
        const member = message.guild?.members.cache.get(user.id);
        if (!member) throw new Error('Unable to get the member');
        const memberSettings = await member.settings();
        try {
            await Members.updateOne({ userID: user.id }, { notes: [...memberSettings.notes, note] });
        } catch (e) {
            throw new Error(e);
        }
