...
module.exports = class extends Command {
    constructor(file: any) {
        super(file, {
            ...
            aliases: ['addrep'],
            cooldown: 0,
            description: `Remove reputation in bulk to a user`,
            enabled: true,
            ignoredInhibitors: [],
    ...
    async run(message: Message, args: string[], client: HozolClient) {
        ...
        if (!args[0]) throw new Error('Please provide an user you would like to remove reps to');
        if (!args[1] || isNaN(parseInt(args[1])) || parseInt(args[0]) > 0)
        ...
        console.log(before);
        const success = await target.removeReputation(parseInt(args[1]));
        const reps = await (await target.profile()).reputation;
        console.log(reps);
        if (success) {
            const embed = new MessageEmbed()
                .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                .setTitle(`Removed Reps to ${target.user.username}`)
            ...
            throw new Error('An Error Ocurred remocing reputation to the user');
        }
    }
};
